...
def LPsolver(c_kij, e_kij, M, a_j, job_task_mapping, fix_kij_tuple_list, init_x_kij, w_kij, precedence, b_sj):
    ...
    model.setObjective(gp.quicksum(
        la_0[k, i, j] + math.pow(1.1, c_kij[k][i][j] + e_kij[k][i][j] + w_kij[k][i][j]) * la_1[k, i, j] for k, i, j in
        kij_tuple_list if
    ...
    for k in range(max_k):
        for i in range(max_i):
            if i in job_task_mapping[k]:
                ...
                if cur_task_sum_equal_1_flag is True and pre_tasks_sum_equal_1_flag is True:
                    model.addConstr(
    ...
    for k in range(max_k):
        #for i1 in range(max_i):
            for i1 in job_task_mapping[k]:
                for j1 in range(max_j):
                    #for i2 in range(max_i):
                        for i2 in job_task_mapping[k]:
